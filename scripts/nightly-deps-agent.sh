#!/usr/bin/env bash
# nightly-deps-agent.sh — nightly runner for the calc Android project.
#
...
REPO="${NIGHTLY_DEPS_REPO:-/home/user/calc}"
STATE_DIR="${NIGHTLY_DEPS_STATE:-$HOME/.local/share/calc/nightly-deps-agent}"
...
directive="Read the canonical prompt at $PROMPT_FILE (also copied to $session_prompt on this host) and execute it fully and autonomously, without ever asking for confirmation or entering plan mode. Repo: $REPO on branch master. Touch $DONE_FILE when fully done so the runner can exit."
